...
export default function SideLettersScreen() {
  ...
  useEffect(() => {
    ...
    if (!session) {
      console.log("[SideLetters] No active session, redirecting to login");
    ...
    async function loadDocuments() {
      try {
        ...
        const result = await fetchDocuments({
          document_category: "side_letter", // As per requirements
          is_latest: true,
          is_deleted: false,
          page: currentPage,
          limit: 10,
        ...
        console.error("[SideLetters] Error in loadDocuments:", err);
  ...
  const handleSelectDocument = async (document: Document) => {
    try {
      ...
      if (versionsResult.error) {
        console.error("[SideLetters] Error fetching versions:", versionsResult.error);
      } else {
      ...
      console.error("[SideLetters] Error selecting document:", error);
    }
  };
  ...
  const handleDownload = async (documentId: string) => {
    try {
      ...
      await downloadFromSupabase(bucketName, document.storage_path, {
        autoOpen: true,
        onError: (error) => {
          console.error("[SideLetters] Download error:", error);
        },
      });
    } catch (error) {
      console.error("[SideLetters] Error downloading document:", error);
    }
  };
  ...
  if (isLoading && !documents.length) {
    return (
      <ThemedView style={styles.container}>
        <ThemedText>Loading side letters...</ThemedText>
      </ThemedView>
    );
  }
  ...
  if (error) {
    return (
      <ThemedView style={styles.container}>
        ...
        <ThemedText style={styles.errorSubtext}>An error occurred loading side letters</ThemedText>
      </ThemedView>
    );
  }
  ...
  return (
    <ThemedScrollView style={styles.container}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.title}>Side Letters</ThemedText>
        <ThemedText style={styles.subtitle}>View side letters and supplemental agreements</ThemedText>
      </ThemedView>

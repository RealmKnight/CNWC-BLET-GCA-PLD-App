...
export async function handler() {
    try {
        ...
        for (const email of failedEmails) {
            try {
                ...
                try {
                    ...
                    console.error(
                        `Primary email service failed: ${primaryError.message}`,
                    );
                    ...
                    if (email.retry_count >= 2) {
                      ...
                      ${
                            ...
                            console.error(
                                `Backup email service failed: ${backupError.message}`,
                            );
                            throw new Error(
                ...
                if (shouldSendFallback && requestData?.members?.division_id) {
                    ...
                    try {
                        ...
                        if (divisionAdmins && divisionAdmins.length > 0) {
                            const message =
                                `Action required: ${requestData.members.name}'s ${requestData.leave_type} request for $
                {requestData.request_date} needs processing. Email delivery failed.`;
                            for (const admin of divisionAdmins) {
                                await sendAdminFallbackNotification(
                                    supabase,
                                    admin.id,
                                    message,
                                );
        ...
        return new Response(
            JSON.stringify({ success: true, processed: failedEmails.length }),
        );
    } catch (error) {
